...
const TEMPLATE_PACKS = [
  {
    ...
    habits: [
      { habit_name: 'Drink 500ml Water', category: 'Health', target_frequency: 'Daily', tracking_type: 'quantity', target_quantity: 0.5, unit: 'liters', time_of_day: 'morning', color: '#3b82f6', icon: 'Droplets' },
  ...
  },
  {
    ...
    habits: [
      ...
      { habit_name: 'Read 20 Pages', category: 'Learning', target_frequency: 'Daily', tracking_type: 'quantity', target_quantity: 20, unit: 'pages', time_of_day: 'evening', color: '#3b82f6', icon: 'Book' },
    ]
  }
];
        ...
        <div className="space-y-3.5 max-h-96 overflow-y-auto pr-1">
          {TEMPLATE_PACKS.map(pack => {
            ...
            return (
              <div
                key={pack.id}
                className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/80 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow"
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold shadow-sm transition-colors flex-shrink-0"
                  >
                    <Plus size={14} /> Add Pack
                  </button>
                </div>

                <div className="mt-3 pt-2.5 border-t border-gray-50 dark:border-gray-700/50 flex flex-wrap gap-2">
                  {pack.habits.map((h, idx) => (
                    <span
                      key={idx}
                      className="px-2.5 py-1 rounded-lg text-[11px] font-medium bg-gray-50 dark:bg-gray-700/60 text-gray-700 dark:text-gray-300 flex items-center gap-1"
                    >
